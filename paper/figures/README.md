...
| `hitl-decision-conditions.drawio` | HITL 触发条件与决策逻辑 | 第 4 章 | 图 4-4 |
| `workflow-flowchart.drawio` | 六阶段 de novo 工作流与恢复控制 | 第 4 章 | 图 4-5 |
| `algorithm-loop.drawio` | CEBRA-WP 算法闭环 | 第 4 章 | 图 4-6 |
| `uml-contracts.drawio` | 核心数据契约 UML | 第 4 章 | 图 4-7 |
| `t1-trpcage-instance-walkthrough.drawio` | t1 任务实例走查 | 第 4 章 | 图 4-8 |
| `runtime-sequence.drawio` | 运行时执行序列 | 第 5 章 | 图 5-1 |
| `workflow-swimlane.drawio` | 泳道式工作流总览 | 第 5 章 | 图 5-2 |
| `technical-route.drawio` | 技术路线概览 | 第 2 章 | — |
| `experiment-design-framework.drawio` | 实验设计框架 | 第 7 章 | 图 7-1 |
| `recovery-path-comparison-timeline.drawio` | 恢复路径对比 | 第 7 章 | 图 7-2 |

说明：`hitl-decision-conditions.drawio` 当前只有 PNG 渲染产物，终稿可使用 `hitl-decision-conditions.drawio.png`，或在最终导出时补充 SVG。
