...
\begin{longtable}{@{}
  ...
  >{\centering\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2500}}
  >{\centering\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2500}}@{}}
\caption{后端核心模块与论文架构层对应关系}\label{tab:5-1}\\
\toprule
...
后端提供
\path|/task-intakes/schema|、\path|/task-intakes|、\path|/task-intakes/{id}|
和 \path|/task-intakes/{id}/confirm|
...
来源：\path|../thesis-project.dev/src/api/main.py:166|

\begin{lstlisting}[style=paperpython]
...
正式任务创建后，系统通过任务生命周期接口暴露任务状态、事件和报告。\path|GET /tasks/{task_id}|
...
\path|GET /tasks/{task_id}/events|
返回事件时间线。该接口直接读取事件日志，而不是只依赖内存任务表。因此，只要日志文件存在，服务重启后仍可还原任务的状态迁移、步骤执行、等待进入、决策应用和等待退出等事件。\path|GET /tasks/{task_id}/report|
...
建模。\path|GET /pending-actions|
返回待审查任务摘要，\path|GET /pending-actions/{id}|
返回候选详情，\path|POST /pending-actions/{id}/decision|
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.95\linewidth,height=0.70\textheight,keepaspectratio]{generated-figures/runtime-sequence.drawio.pdf}
  \caption{运行时执行序列}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.95\linewidth,height=0.70\textheight,keepaspectratio]{generated-figures/workflow-swimlane.drawio.pdf}
  \caption{工作流泳道式模块协作}
...
\subsection{本章小结}\label{system-implementation-summary}
