...
\item
  RQ-A3：Lite belief-state / 轻量信念状态相比 \path|dynamic_no_belief_state|
  是否提供额外机制信息。
\item
...
实验采用四组内部消融设计：\path|static_top1|、\path|fixed_threshold_gate|、\path|dynamic_no_belief_state|
和 \path|lite_belief_state|。四组策略按照运行时介入深度递进：静态
Top-1
只选择静态最高分候选；固定阈值门控组保留阈值门控和局部修补触发；动态观测组保留恢复链路但不启用显式
Lite belief-state / 轻量信念状态；\path|lite_belief_state|
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.95\linewidth,height=0.70\textheight,keepaspectratio]{generated-figures/experiment-design-framework.drawio.pdf}
  \caption{实验设计框架}
  \label{fig:experiment-design-framework}
\end{figure}

图 7-1 的作用是限定本章统计结果的来源。后续表 7-1 至表 7-8 均来自
\path|thesis-final-v1-001|
实验矩阵及其配套日志、快照和聚合产物；对应内部证据入口为
\path|../thesis-project.dev/docs/experiment/thesis-final-v1-results.md|
和
\path|../thesis-project.dev/output/experiment/thesis-final-matrix/thesis-final-v1-001/|。

{%
\begin{longtable}{@{}
  >{\centering\arraybackslash}p{(\linewidth - 2\tabcolsep) * \real{0.5000}}
  >{\centering\arraybackslash}p{(\linewidth - 2\tabcolsep) * \real{0.5000}}@{}}
\caption{实验矩阵配置表}\label{tab:7-1}\\
\toprule
...
\bottomrule
\endlastfoot
run\_id & \path|thesis-final-v1-001| \\
freeze\_id & \path|issue209-baseline-freeze-20260326| \\
planner\_provider & \path|deepseek-v4-pro| \\
任务覆盖 & 12 个 task\_keys，覆盖 T1 至 T8 共 8 类场景 \\
策略组 & \path|static_top1| / \path|fixed_threshold_gate| /
\path|dynamic_no_belief_state| / \path|lite_belief_state| \\
repeats &
\path|low_cost_first=2|，\path|standard=2|，\path|high_cost_sensitive=1| \\
...
\path|../thesis-project.dev/docs/experiment/final-thesis-experiment-design.md|
和
\path|../thesis-project.dev/docs/experiment/thesis-final-v1-results.md|。该任务集使实验同时包含低成本成功路径、中等压力路径和高代价结构预测路径。
...
\caption{四组消融主实验结果}\label{tab:7-2}\\
\toprule
\begin{minipage}[b]{\linewidth}\centering
指标
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
\begin{tabular}[c]{@{}c@{}}\texttt{static}\\\texttt{\_top1}\end{tabular}
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
\begin{tabular}[c]{@{}c@{}}\texttt{fixed}\\\texttt{\_threshold}\\\texttt{\_gate}\end{tabular}
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
\begin{tabular}[c]{@{}c@{}}\texttt{dynamic}\\\texttt{\_no\_belief}\\\texttt{\_state}\end{tabular}
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
\begin{tabular}[c]{@{}c@{}}\texttt{lite}\\\texttt{\_belief}\\\texttt{\_state}\end{tabular}
\end{minipage} \\
\midrule
\endfirsthead
...
DONE & 21 & 20 & 20 & 20 \\
FAILED & 0 & 1 & 1 & 1 \\
\path|success_rate| & 1.0000 & 0.9524 & 0.9524 & 0.9524 \\
\path|first_pass_success_rate| & 1.0000 & 0.9048 & 0.9524 & 0.9524 \\
\path|schema_valid_rate| & 1.0000 & 1.0000 & 0.9524 & 1.0000 \\
\path|executable_plan_rate| & 1.0000 & 0.9524 & 1.0000 & 1.0000 \\
\path|waiting_chain_complete_rate| & 1.0000 & 1.0000 & 1.0000 & 1.0000 \\
\path|failure_traceable_rate| & 1.0000 & 1.0000 & 1.0000 & 1.0000 \\
\path|runtime_state_observable_rate| & 0.0000 & 0.0000 & 0.0000 & 1.0000 \\
\path|patch_events_mean| & 0.0000 & 0.2857 & 0.0000 & 0.0000 \\
\path|replan_events_mean| & 0.0000 & 0.0000 & 0.0000 & 0.0000 \\
\path|suffix_replan_events_mean| & 0.0000 & 0.0000 & 0.0000 & 0.0000 \\
\path|high_cost_call_mean| & 1.0000 & 1.3333 & 0.9524 & 0.9524 \\
\path|high_cost_call_total| & 21 & 28 & 20 & 20 \\
\path|duration_ms_mean| & 241,905 & 300,238 & 226,571 & 272,095 \\
\path|action_continue_mean| & 1.0952 & 1.5238 & 0.9524 & 0.9524 \\
\path|action_patch_local_mean| & 0.0000 & 0.2857 & 0.0000 & 0.0000 \\
\end{longtable}
}
...
\caption{按任务难度与预算分层的成功率}\label{tab:7-3}\\
\toprule
...
\begin{longtable}{@{}
  ...
  >{\centering\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.4100}}@{}}
\caption{机制增量配对对比}\label{tab:7-4}\\
\toprule
\begin{minipage}[b]{\linewidth}\centering
对比
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
...
\bottomrule
\endlastfoot
\begin{tabular}[c]{@{}c@{}}static\\→fixed\end{tabular} & \path|success_rate| & -0.0476 & 21 & fixed 组多 1 个失败。 \\
\begin{tabular}[c]{@{}c@{}}static\\→fixed\end{tabular} & \path|first_pass_success_rate| & -0.0952 & 21 & fixed
组首次成功率下降。 \\
\begin{tabular}[c]{@{}c@{}}static\\→fixed\end{tabular} & \path|patch_event_count| & +0.2857 & 21 &
\path|fixed_threshold_gate| 组触发真实局部修补。 \\
\begin{tabular}[c]{@{}c@{}}static\\→fixed\end{tabular} & \path|high_cost_call_count| & +0.3333 & 21 & fixed
组高代价调用增加。 \\
\begin{tabular}[c]{@{}c@{}}fixed\\→dynamic\end{tabular} & \path|high_cost_call_count| & -0.3810 & 21 & dynamic
组高代价调用低于 fixed。 \\
\begin{tabular}[c]{@{}c@{}}fixed\\→dynamic\end{tabular} & \path|duration_ms| & -73,667 & 21 & dynamic 组平均耗时低于
fixed。 \\
\begin{tabular}[c]{@{}c@{}}dynamic\\→lite\end{tabular} & \path|schema_valid_rate| & +0.0476 & 21 & lite 组 schema valid
恢复到 1.0000。 \\
\begin{tabular}[c]{@{}c@{}}dynamic\\→lite\end{tabular} & \path|high_cost_call_count| & 0.0000 & 21 &
两组高代价调用均值相同。 \\
\begin{tabular}[c]{@{}c@{}}dynamic\\→lite\end{tabular} & \path|duration_ms| & +45,524 & 21 & lite 组平均耗时高于
dynamic。 \\
\end{longtable}
}
...
\begin{longtable}{@{}
  ...
  >{\centering\arraybackslash}p{(\linewidth - 10\tabcolsep) * \real{0.1739}}
  >{\centering\arraybackslash}p{(\linewidth - 10\tabcolsep) * \real{0.1739}}@{}}
\caption{高代价调用与运行时间对比}\label{tab:7-5}\\
\toprule
\begin{minipage}[b]{\linewidth}\centering
组
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
...
\begin{longtable}{@{}
  ...
  >{\centering\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.3333}}
  >{\centering\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.3333}}@{}}
\caption{Lite belief-state 机制可观测性对比}\label{tab:7-6-lite-belief-state}\\
\toprule
\begin{minipage}[b]{\linewidth}\centering
特征
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.95\linewidth,height=0.70\textheight,keepaspectratio]{generated-figures/recovery-path-comparison-timeline.drawio.pdf}
  \caption{固定修补与 Lite belief-state 重排序的恢复路径对比}
  \label{fig:recovery-path-comparison-timeline}
\end{figure}
...
\begin{longtable}{@{}
  ...
  >{\centering\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2500}}
  >{\centering\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2500}}@{}}
\caption{实验结果证据产物索引}\label{tab:7-8}\\
\toprule
\begin{minipage}[b]{\linewidth}\centering
证据产物
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
