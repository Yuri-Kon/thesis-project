...
## 附录：PLM 初始序列生成（ProtGPT2）

本节补充 PLM 初始序列生成的实现约定，供后续适配器与 KG 节点落地参考：
...
- 典型输出：
  - `sequence`: 选定候选序列
  - `candidates`: 序列列表（可包含 perplexity/score）
  - `artifacts`: FASTA/JSON 文件路径
- 执行方式：
  - `execution = "python"`：本地 Hugging Face 推理（Transformers + Torch）
  - `execution = "remote_model_service"`：通过 `SSHModelInvocationService` 走远程主机
- 运行假设：
  - 本地路径：已安装 `transformers`/`torch`，权重可从 HF 缓存或镜像加载
  - SSH 路径：远端可 SSH 访问，已部署 ProtGPT2 推理脚本与模型权重
- 与下游衔接：
  - `protgpt2.outputs.sequence` → `esmfold.inputs.sequence`
  - 结构确定后可选接 `protein_mpnn` 做重设计
