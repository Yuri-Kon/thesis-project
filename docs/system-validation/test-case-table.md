...
证据编号详见 `docs/system-validation/evidence-index.md`。本表中的 `FIG-SV-*`、`EVD-API-*`、`EVD-TEST-*`、`EVD-CLI-*`、`EVD-EXP-*`、`EVD-LOG-*` 均来自该索引，可直接作为论文正文、图表和附录的引用编号。
...
|---|---|---|---|---|---|---|
| TC-S01 | 环境与能力就绪验证 | EXP-S1 | SV-01、SV-27 | API / 环境 | 通过 | EVD-API-01、EVD-API-02、EVD-TEST-01 |
| TC-S02 | 任务录入与 API 合约验证 | EXP-S2 | SV-02、SV-03、SV-04、SV-07、SV-25、SV-26 | API / 自动化 | 通过 | EVD-TEST-01、EVD-CLI-01、EVD-API-01、EVD-API-02 |
| TC-S03 | 任务创建与计划候选验证 | EXP-S2、EXP-S5 | SV-08、SV-09、SV-10 | API / 集成 | 通过 | EVD-TEST-02、EVD-LOG-04、EVD-EXP-01 |
| TC-S04 | HITL 决策与 PendingAction 一致性验证 | EXP-S3 | SV-10、SV-11、SV-12、SV-13、SV-15 | 单元 / 集成 / API | 通过 | EVD-TEST-02、FIG-SV-15 |
| TC-S05 | FSM 状态迁移与终态不可变验证 | EXP-S3 | SV-14、SV-23、SV-24 | 单元 / 集成 | 通过 | EVD-TEST-02、EVD-LOG-03 |
| TC-S06 | 快照恢复与运行时状态边界验证 | EXP-S3 | SV-14、SV-28、SV-29 | 单元 / 集成 | 通过 | EVD-TEST-02、EVD-LOG-01、EVD-LOG-02、EVD-LOG-03 |
| TC-S07 | Web 关键页面可用性验证 | EXP-S4 | SV-01、SV-30 | UI / 手工 / smoke | 通过 | FIG-SV-01 至 FIG-SV-17、EVD-TEST-01 |
| TC-S08 | CLI 关键命令可用性验证 | EXP-S4 | SV-02、SV-03、SV-25、SV-26、SV-30 | CLI / 手工 / 自动化 | 部分通过 | EVD-TEST-04、EVD-CLI-01、EVD-CLI-02、EVD-CLI-03、EVD-CLI-04 |
| TC-S09 | 正常端到端任务流程验证 | EXP-S5 | SV-16、SV-25、SV-26 | 集成 / API | 通过 | EVD-EXP-01、EVD-LOG-05、FIG-SV-16、FIG-SV-17 |
| TC-S10 | 异常输入与安全 warn/block 验证 | EXP-S6 | SV-04、SV-05、SV-06、SV-11、SV-13、SV-21、SV-24 | 单元 / API | 通过 | EVD-TEST-03、FIG-SV-10 |
| TC-S11 | 工具链执行与 I/O 边界验证 | EXP-S5、EXP-S6 | SV-16、SV-17 | 集成 / 单元 | 通过 | EVD-TEST-03、EVD-EXP-01、EVD-API-02 |
| TC-S12 | 失败恢复 retry -> patch -> replan 验证 | EXP-S7 | SV-18、SV-19、SV-20、SV-22 | 单元 / 集成 | 通过 | EVD-TEST-03、EVD-LOG-01、EVD-LOG-02、EVD-LOG-03 |
| TC-S13 | 恢复止损与审计链路验证 | EXP-S7 | SV-21、SV-23、SV-25 | 集成 / API | 通过 | EVD-TEST-02、EVD-TEST-03、EVD-LOG-03、EVD-LOG-04 |

## 2.1 用例到证据编号映射（论文引用版）

| 用例 | 输入记录 | 输出记录 | 预期/实际判定 | 主要证据编号 | 备注 |
|---|---|---|---|---|---|
| TC-S01 | 启动 API 并请求 `/health`、`/capabilities/readiness` | health/readiness JSON | API 可访问；能力降级有明确说明 | EVD-API-01、EVD-API-02、EVD-TEST-01 | 可写入环境验证小节 |
| TC-S02 | task-intake schema、任务创建和异常输入请求 | API 自动化断言、CLI schema 输出 | 合约字段、错误响应和边界条件符合预期 | EVD-TEST-01、EVD-CLI-01 | 典型 task JSON 仍建议补充归档 |
| TC-S03 | 创建任务并触发计划候选 | Plan/PendingAction/EventLog | Planner 只产出候选，等待人工确认时写入 pending action | EVD-TEST-02、EVD-LOG-04 | 可支撑“计划生成不越权执行” |
| TC-S04 | PendingAction accept/reject/cancel 与错误绑定 | 决策结果、状态迁移、UI 候选区域 | 非法决策拒绝；合法决策仅应用一次 | EVD-TEST-02、FIG-SV-15 | 可支撑 HITL 一致性 |
| TC-S05 | 状态迁移、终态再变更尝试 | EventLog、状态机断言 | 终态不可变；等待态前有日志与快照 | EVD-TEST-02、EVD-LOG-03 | 可支撑 FSM 控制 |
| TC-S06 | 等待态/运行态快照恢复 | Snapshot、恢复后状态 | 恢复不自动越过人工确认；runtime_state 不污染 Plan | EVD-TEST-02、EVD-LOG-01、EVD-LOG-02、EVD-LOG-03 | 可支撑恢复一致性 |
| TC-S07 | Dashboard、Task Builder、Task Detail 页面访问 | 17 张前端截图、Web smoke 日志 | 关键页面可加载并展示同一任务证据链 | FIG-SV-01 至 FIG-SV-17、EVD-TEST-01 | Timeline 单独截图待补 |
| TC-S08 | CLI intake schema、task show、timeline/report 命令 | CLI 输出日志、pytest CLI 日志 | schema 和 task show 可用；timeline/report CLI 为当前限制 | EVD-TEST-04、EVD-CLI-01 至 EVD-CLI-04 | 论文中应写为“部分通过” |
| TC-S09 | t9 clean 端到端运行与报告查询 | 16-run 聚合、报告 JSON | 4 任务 × 4 组策略均 DONE，report 合约通过 | EVD-EXP-01、EVD-LOG-05 | 正式 84-run 完成后再更新结论 |
| TC-S10 | forbidden motif、安全 warn/block、异常输入 | SafetyResult、StepRunner 日志、API 断言 | block 阻止工具执行；warn 记录风险并继续 | EVD-TEST-03、FIG-SV-10 | 可支撑安全边界 |
| TC-S11 | 工具链执行、候选 schema/I-O 边界 | StepResult、metrics、候选验证结果 | 正常工具产出 artifacts；错误候选被拒绝 | EVD-TEST-03、EVD-EXP-01、EVD-API-02 | 可支撑工具链可执行性 |
| TC-S12 | retry exhausted、patch、replan 场景 | EventLog、Snapshot、恢复指标 | retry 后进入恢复流程，不直接 FAILED | EVD-TEST-03、EVD-LOG-01、EVD-LOG-02、EVD-LOG-03 | Plan diff 样本建议后续集中归档 |
| TC-S13 | safety block 或 terminal_stop 场景 | FAILED TaskRecord、EventLog 审计链 | terminal_stop 后进入 FAILED，事件链可追溯 | EVD-TEST-02、EVD-TEST-03、EVD-LOG-03、EVD-LOG-04 | 可支撑止损机制 |
...
| TC-S02 | 测试环境依赖安装完成；API 测试可运行 | 1. 运行 `uv run pytest tests/api/test_api_endpoints.py -q`。<br>2. 手工或脚本请求 task-intake、task、events、report 相关接口。<br>3. 记录正常响应与异常响应。 | intake schema 可返回字段注册表；正常输入可创建 intake/task；缺字段、互斥创建模式、未完成 report 等异常返回明确错误；events 支持查询。 | 2026-05-10 focused suite 通过；API 合约由 `pytest-api-web.log` 归档；2026-05-10 补充：task-intake schema、task detail、events、report、pending 5 个典型 API 响应已归档为独立 JSON 证据（EVD-API-03 至 EVD-API-07）。 | EVD-TEST-01；EVD-API-01；EVD-API-02；EVD-CLI-01；EVD-API-03 至 EVD-API-07 | 通过 |
...
| TC-S04 | 存在进入等待态的任务和有效 PendingAction | 1. 运行 decision 相关单测。<br>2. 对 accept 缺少 `selected_candidate_id` 的请求进行验证。<br>3. 对已决策 PendingAction 重复提交。<br>4. 对错误 task/pending 绑定提交 decision。 | accept 缺少候选 ID 被拒绝；重复 decision 返回冲突；错误绑定被拒绝；等待态下 Executor 不继续执行工具；合法 Decision 只应用一次。 | 2026-05-10 decision、status、snapshot 与 EventLog focused suite 通过。 | EVD-TEST-02；FIG-SV-15 | 通过 |
| TC-S05 | 状态迁移单测和事件日志测试可运行 | 1. 运行 `tests/unit/test_status_transition.py`、`tests/unit/test_event_log.py`。<br>2. 构造或复用 DONE、FAILED、CANCELLED 终态任务。<br>3. 尝试再次提交状态变更或 decision。 | 进入 `WAITING_*` 前写入 snapshot 和 `WAITING_ENTER`；terminal stop 接受后进入 `FAILED`；DONE/FAILED/CANCELLED 不可再变更。 | 2026-05-10 FSM/HITL/snapshot focused suite 通过，S6 原始日志提供 terminal stop 和恢复审计链样本。 | EVD-TEST-02；EVD-LOG-03 | 通过 |
| TC-S06 | snapshot store 可写；存在等待态或运行态任务 | 1. 运行 `tests/unit/test_task_snapshot.py` 和 `tests/integration/test_snapshot_recovery.py`。<br>2. 保存等待态 snapshot。<br>3. 执行恢复流程。<br>4. 对比 Plan 与 snapshot artifacts。 | snapshot 可恢复 plan、completed steps、pending action 和 runtime_state；恢复到 `WAITING_*` 后不自动推进；runtime_state 仅在 snapshot artifacts 持久化，不污染 Plan。 | 2026-05-10 FSM/HITL/snapshot focused suite 通过，retry/patch 和 S6 运行日志提供 snapshot 样本索引。 | EVD-TEST-02；EVD-LOG-01；EVD-LOG-02；EVD-LOG-03 | 通过 |
| TC-S07 | API 服务启动；前端静态资源可访问；至少存在一个测试任务 | 1. 打开 `/ui`。<br>2. 打开 `/ui/task-builder` 并完成 intake 创建/确认。<br>3. 打开 `/ui/tasks/{task_id}`。<br>4. 查看 timeline 和 pending review 面板。 | Dashboard、Task Builder、Task Detail、Timeline 可加载；同一 task_id 的状态、pending_action_id、事件链可在页面展示；人工确认入口可见。 | 2026-05-10 执行：Dashboard/TaskBuilder/TaskDetail 页面均可正常加载；中文界面变体截图一并留存；2026-05-10 补充：单独 Timeline 页面截图（`/ui/tasks/demo_structure_viewer/events`），展示 11 个事件（STATE_TRANSITION ×4、STEP_FINISHED ×2、PENDING_ACTION_CREATED、WAITING_ENTER、DECISION_APPLIED、WAITING_EXIT、STATE_TRANSITION ×2），覆盖 CREATED→DONE 全生命周期。 | `docs/system-validation/06-ui-screenshots/` 共 18 张 PNG（Dashboard、Task Builder、Task Detail 中英文变体 + Timeline 单独截图）；EVD-API-05（events JSON） | 通过 |
...
当前已完成集中编号的证据见 `docs/system-validation/evidence-index.md`：18 张前端截图（含单独 Timeline）、7 个 API JSON（含 task/detail/events/report/pending）、4 组 pytest focused 日志、4 个 CLI 输出日志、2 个已完成 smoke/clean 实验目录、若干 EventLog/Snapshot/Report 原始产物，以及 2 组 EventLog+Snapshot 样本已复制到 `04-data-consistency/`。
...
| 端到端流程验证 | TC-S09、TC-S11 | 正常任务执行、StepResult、DesignResult、工具 I/O | 通过 | t8 四组 smoke（4/4 DONE）+ t9 四组 clean run（16/16 DONE），覆盖 4 类任务 × 4 组策略；API report 与候选 I/O 边界 focused tests 通过 |
...
| 失败恢复验证 | TC-S12、TC-S13 | retry、patch、replan、terminal stop、恢复审计链 | 通过 | TC-S12 恢复专项通过；TC-S13 terminal stop 审计链 tests 通过 |
