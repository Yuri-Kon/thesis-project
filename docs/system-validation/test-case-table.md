...
| TC-S02 | 测试环境依赖安装完成；API 测试可运行 | 1. 运行 `uv run pytest tests/api/test_api_endpoints.py -q`。<br>2. 手工或脚本请求 task-intake、task、events、report 相关接口。<br>3. 记录正常响应与异常响应。 | intake schema 可返回字段注册表；正常输入可创建 intake/task；缺字段、互斥创建模式、未完成 report 等异常返回明确错误；events 支持查询。 | 2026-05-10 focused suite 通过；API 合约由 `pytest-api-web.log` 归档，health/readiness JSON 已归档；典型 task/pending/events/report 响应仍建议补充为独立 JSON。 | EVD-TEST-01；EVD-API-01；EVD-API-02；EVD-CLI-01 | 通过 |
...
当前已完成集中编号的证据见 `docs/system-validation/evidence-index.md`：17 张前端截图、2 个 API JSON、4 组 pytest focused 日志、4 个 CLI 输出日志、2 个已完成 smoke/clean 实验目录、若干 EventLog/Snapshot/Report 原始产物。表中建议路径仍作为后续补充归档规范，尤其用于补齐典型 task/pending/events/report API JSON、单独 Timeline 截图和 84-run 完整聚合产物。
