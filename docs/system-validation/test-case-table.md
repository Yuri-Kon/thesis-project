...
| TC-S02 | 任务录入与 API 合约验证 | EXP-S2 | SV-02、SV-03、SV-04、SV-07、SV-25、SV-26 | API / 自动化 | 待执行 | pytest log、API 响应 JSON |
| TC-S03 | 任务创建与计划候选验证 | EXP-S2、EXP-S5 | SV-08、SV-09、SV-10 | API / 集成 | 待执行 | task JSON、pending action JSON、候选 JSON |
| TC-S04 | HITL 决策与 PendingAction 一致性验证 | EXP-S3 | SV-10、SV-11、SV-12、SV-13、SV-15 | 单元 / 集成 / API | 待执行 | decision 测试日志、EventLog、pending action JSON |
| TC-S05 | FSM 状态迁移与终态不可变验证 | EXP-S3 | SV-14、SV-23、SV-24 | 单元 / 集成 | 待执行 | 状态迁移测试日志、EventLog |
...
|---|---|---|---|---|---|---|
| TC-S01 | API 服务可启动；本地配置文件可读取 | 1. 启动 `uv run uvicorn src.api.main:app --reload`。<br>2. 请求 `GET /health`。<br>3. 请求 `GET /capabilities/readiness`。<br>4. 若远程服务不可用，记录 degraded reason。 | `/health` 返回 `status=ok`；readiness 返回 capability、available_tools、blocked_tools、degraded_reasons；远程不可用时给出明确原因。 | 待执行 | `docs/system-validation/00-environment/server-startup.log`；`docs/system-validation/05-api-results/health.json`；`docs/system-validation/05-api-results/readiness.json` | 待验证 |
| TC-S02 | 测试环境依赖安装完成；API 测试可运行 | 1. 运行 `uv run pytest tests/api/test_api_endpoints.py -q`。<br>2. 手工或脚本请求 task-intake、task、events、report 相关接口。<br>3. 记录正常响应与异常响应。 | intake schema 可返回字段注册表；正常输入可创建 intake/task；缺字段、互斥创建模式、未完成 report 等异常返回明确错误；events 支持查询。 | 待执行 | `docs/system-validation/07-test-runs/pytest-api.log`；`docs/system-validation/05-api-results/task-intakes/`；`docs/system-validation/05-api-results/tasks/` | 待验证 |
| TC-S03 | Planner、ToolKG 和任务创建入口可用 | 1. 使用自由文本或 confirmed task spec 创建任务。<br>2. 触发计划生成。<br>3. 查询 task 与 pending action。 | Planner 只生成候选，不执行工具、不直接改状态；Plan candidate 包含候选 ID、payload、评分、风险、成本、解释；需要确认时进入 `WAITING_PLAN_CONFIRM` 并生成 `PendingAction(plan_confirm)`。 | 待执行 | `docs/system-validation/01-core-flows/intake-confirm-flow/`；`docs/system-validation/02-hitl/plan-confirm/` | 待验证 |
| TC-S04 | 存在进入等待态的任务和有效 PendingAction | 1. 运行 decision 相关单测。<br>2. 对 accept 缺少 `selected_candidate_id` 的请求进行验证。<br>3. 对已决策 PendingAction 重复提交。<br>4. 对错误 task/pending 绑定提交 decision。 | accept 缺少候选 ID 被拒绝；重复 decision 返回冲突；错误绑定被拒绝；等待态下 Executor 不继续执行工具；合法 Decision 只应用一次。 | 待执行 | `docs/system-validation/07-test-runs/pytest-decision.log`；`docs/system-validation/05-api-results/decisions/`；`docs/system-validation/04-data-consistency/event-logs/` | 待验证 |
| TC-S05 | 状态迁移单测和事件日志测试可运行 | 1. 运行 `tests/unit/test_status_transition.py`、`tests/unit/test_event_log.py`。<br>2. 构造或复用 DONE、FAILED、CANCELLED 终态任务。<br>3. 尝试再次提交状态变更或 decision。 | 进入 `WAITING_*` 前写入 snapshot 和 `WAITING_ENTER`；terminal stop 接受后进入 `FAILED`；DONE/FAILED/CANCELLED 不可再变更。 | 待执行 | `docs/system-validation/07-test-runs/pytest-fsm.log`；`docs/system-validation/04-data-consistency/event-logs/` | 待验证 |
...
|---|---|---|---|---|
| 环境与能力验证 | TC-S01 | API 健康检查、能力目录、readiness 降级说明 | 待填 | health/readiness JSON |
