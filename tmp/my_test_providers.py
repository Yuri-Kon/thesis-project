...
Usage: uv run python tmp/my_test_providers.py [alias ...]
       uv run python tmp/my_test_providers.py glm-5 deepseek-v4-pro

Checks 1) API key avaliable 2) provider instantiated 3) call_planner() round-trip with a minimal task
"""
...
PROJECT_ROOT = Path(__file__).resolve().parents[1]  # __file__ 表示当前脚本文件的路径
sys.path.insert(0, str(PROJECT_ROOT))  # 把项目根目录插入到 python 模块搜索路径的最前面
...
def check_provider(alias: str, settings: object) -> tuple[bool, str]:
    ...
    try:
        ...
        return False, f"creata error: {exce}"
