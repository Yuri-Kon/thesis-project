...
def test_benchmark_acceptance_gate_passes_current_freeze_config(tmp_path: Path) -> None:
    report, output_dir = run_issue200_acceptance_gate(
        ...
        freeze_id="benchmark-gate-pass",
    )

    assert report["overall_status"] == "pass"
    assert report["summary"]["block_count"] == 0
    assert (output_dir / "benchmark_acceptance_gate_report.json").exists()
    assert (output_dir / "benchmark_acceptance_gate_summary.md").exists()
    assert (output_dir / "benchmark_gate_summary.json").exists()
    assert (output_dir / "benchmark_gate_blockers.json").exists()
    assert (output_dir / "benchmark_gate_evidence_index.json").exists()
    ...
    gate_summary = json.loads((output_dir / "benchmark_gate_summary.json").read_text(encoding="utf-8"))
    blockers = json.loads((output_dir / "benchmark_gate_blockers.json").read_text(encoding="utf-8"))
    evidence_index = json.loads(
        (output_dir / "benchmark_gate_evidence_index.json").read_text(encoding="utf-8")
    )
    ...
    assert "benchmark-gate-summary" in artifact_ids
    assert "benchmark-gate-blockers" in artifact_ids


def test_benchmark_acceptance_gate_blocks_inconsistent_tool_and_provider_alias(
    tmp_path: Path,
) -> None:
    config = load_json(Path("configs/experiments/benchmark_platform_adapters.json"))
    ...
    report, _ = run_issue200_acceptance_gate(
        ...
        output_root=tmp_path,
        freeze_id="benchmark-gate-block",
    )
...
def test_benchmark_acceptance_gate_cli_returns_nonzero_on_block(tmp_path: Path) -> None:
    config = load_json(Path("configs/experiments/benchmark_platform_adapters.json"))
    broken_config = json.loads(json.dumps(config))
    broken_config["tool_whitelist"]["high_cost_tool_ids"].append("not_in_allowlist")

    config_path = tmp_path / "benchmark-broken-config.json"
    config_path.write_text(json.dumps(broken_config, ensure_ascii=False), encoding="utf-8")

    result = subprocess.run(
        [
            sys.executable,
            "scripts/benchmarks/run_benchmark_acceptance_gate.py",
            "--config",
            str(config_path),
            "--output-root",
            str(tmp_path / "out"),
    ...
    assert "[benchmark-gate] overall_status=block" in result.stdout
    assert "tool_whitelist_known_tools" in result.stdout
    assert "[benchmark-gate] gate_summary=" in result.stdout
