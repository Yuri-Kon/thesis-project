...
def test_issue172_run_manifest_materializes_dry_run_configs(tmp_path: Path) -> None:
    repo_root = Path(__file__).resolve().parents[2]
    config = {
        "issue_id": 172,
        "platform_freeze_config_path": str(
            repo_root / "configs/experiments/issue199_benchmark_platform_adapters.json"
        ),
    ...
    manifest, run_dir = build_issue172_run_manifest(
        config=config,
        config_path=repo_root / "configs/experiments/w16_issue172_horizontal_experiment.json",
        output_root=tmp_path / "out",
        run_id="issue172-dry-run",
        repeats_override=1,
        dry_run=True,
    )

    assert manifest["freeze_id"] == "issue199-platform-freeze-20260412"
...
def test_issue172_evaluator_writes_horizontal_outputs(tmp_path: Path) -> None:
    ...
    assert "Horizontal Experiment Report (E0/E1/E2)" in report
