...
def _load_module():
    module_path = Path(__file__).resolve().parents[2] / "scripts" / "generate_w12_issue174_midterm_pack.py"
    spec = importlib.util.spec_from_file_location("issue174_pack", module_path)
...
def test_build_issue174_pack_marks_horizontal_as_deferred(tmp_path: Path) -> None:
    ...
    result = module.build_issue174_pack(
        ...
        horizontal_note="issue #172 deferred for external dependency",
    )
    ...
    assert "issue #172 deferred for external dependency" in chapter_text
