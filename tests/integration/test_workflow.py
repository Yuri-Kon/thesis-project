...
class TestWorkflowIntegration:
    ...
    def test_run_task_sync_completes_full_workflow(
        self, sample_task: ProteinDesignTask
    ):
        """测试同步任务执行完成完整工作流"""
    ...
    def test_run_task_sync_generates_design_result(
        self, sample_task: ProteinDesignTask
    ):
        """测试任务执行生成设计结果"""
        record = run_task_sync(sample_task)
    ...
    def test_run_task_sync_updates_status_progressively(
        self, sample_task: ProteinDesignTask
    ):
        """测试任务状态逐步更新"""
    ...
    def test_run_task_sync_creates_report_file(
        self, sample_task: ProteinDesignTask, tmp_path: Path
    ):
        """测试任务执行创建报告文件"""
