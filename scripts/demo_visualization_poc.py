...
def write_report_html(
    ...
    html = "\n".join(
        [
            ...
            "  <link",
            "    rel=\"stylesheet\"",
            "    href=\"https://cdn.jsdelivr.net/npm/molstar@3.52.0/build/viewer/molstar.css\"",
            "  />",
            "  <script src=\"https://cdn.jsdelivr.net/npm/molstar@3.52.0/build/viewer/molstar.min.js\"></script>",
            "  <style>",
            ...
            "    #molstar-viewer { width: 100%; height: 520px; }",
            "  </style>",
            "</head>",
            "<body>",
            "  <main>",
            ...
            "      <div id=\"molstar-error\" style=\"display:none; margin-top: 12px; color: #b91c1c;\"></div>",
            "    </section>",
            "  </main>",
            "  <script>",
            ...
            "        showError(",
            "          `Mol* failed to load from CDN. Check network access and reload. ` +",
            ...
            "        };",
            "        const viewerPromise =",
            "          typeof molstar.Viewer.create === 'function'",
            "            ? molstar.Viewer.create('molstar-viewer', options)",
            "            : Promise.resolve(new molstar.Viewer('molstar-viewer', options));",
            "        viewerPromise",
            "          .then((viewer) => viewer.loadStructureFromUrl(pdbUrl, 'pdb'))",
            "          .catch(() => {",
            "            showError(",
            "              `Mol* failed to load. Open the PDB directly: <a href=\"${pdbUrl}\">${pdbUrl}</a>`",
            "            );",
            "          });",
            "      }",
            "    } catch (err) {",
            "      showError(",
            "        `Mol* failed to initialize. Open the PDB directly: <a href=\"${pdbUrl}\">${pdbUrl}</a>`",
            "      );",
            "    }",
            "  </script>",
            "</body>",
