...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate thesis four-group experiment outputs."
    )
    parser.add_argument(
        "--run-manifest-path",
        type=Path,
        required=True,
        help="Path to runs_manifest.json produced by the thesis matrix runner.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
...
def main() -> int:
    ...
    print(f"[thesis-matrix] output_dir={output_dir}")
    print(f"[thesis-matrix] summary_csv={output_dir / 'matrix_metrics_summary.csv'}")
    print(f"[thesis-matrix] rerun_candidates={output_dir / 'rerun_candidates.json'}")
    print(f"[thesis-matrix] evidence_index={output_dir / 'evidence_index.json'}")
    return 0
