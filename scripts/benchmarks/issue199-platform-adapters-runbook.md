# Issue #199：Inspect AI + promptfoo 平台接入与复现 Runbook
...
2. 如果需要远程工具服务，确认 `PLM_REST_BASE_URL` / `OPENFOLD3_REST_BASE_URL` 可访问。
3. 在 `#172` 正式落地时，将相同 `freeze_id / budget / tool whitelist / dataset version` 带入真实横向实验。
