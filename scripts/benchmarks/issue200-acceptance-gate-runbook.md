...
- `configs/experiments/issue199_benchmark_platform_adapters.json`
...
- 该入口会先运行 focused pytest：
  - `tests/unit/test_issue200_acceptance_gate.py`
  - `tests/unit/test_benchmark_platform_adapters.py`
- 然后执行 issue #200 gate runner。
...
- `issue200_acceptance_suite.json`：本地/CI 一键验收总入口结果
- `issue200_acceptance_gate_report.json`：完整 checks report
- `issue200_gate_summary.json`：给 `#172 / #221 / #224` 直接消费的门禁摘要
- `issue200_gate_blockers.json`：阻断项与修复建议清单
- `issue200_gate_evidence_index.json`：按 evidence-index 风格整理的门禁产物索引

## 7. CI 入口

仓库工作流：

- `.github/workflows/issue200-acceptance-gate.yml`

触发方式：

- `workflow_dispatch`
- 对 issue #200 相关配置/脚本/测试文件的 `pull_request`

CI 中执行的入口与本地一致：

```bash
uv run python scripts/benchmarks/run_issue200_acceptance_suite.py \
  --output-root output/experiment/w16-issue200-ci \
  --freeze-id issue200-ci-<run_id>
```
...
- `issue200_gate_summary.json.ready_for_downstream=true`
...
- 优先引用 `issue200_gate_summary.json`
- 修复问题时优先查看 `issue200_gate_blockers.json`
- 需要追溯时查看 `issue200_gate_evidence_index.json`
...
- 检查 workflow 日志中 `output/experiment/w16-issue200-ci` 是否成功生成
