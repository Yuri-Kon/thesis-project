...
- 平台冻结：`configs/experiments/issue199_benchmark_platform_adapters.json`
- 任务冻结：`configs/experiments/w13_issue209_baseline_freeze.json`
- 实验配置：`configs/experiments/w16_issue172_horizontal_experiment.json`
...
```bash
uv run python scripts/run_w16_issue172_horizontal_experiment.py \
  --config configs/experiments/w16_issue172_horizontal_experiment.json \
  --run-id issue172-local-dryrun \
  --output-root /tmp/issue172-horizontal \
  --dry-run
```
...
- `issue172` 的 planner 走 `configs/llm_providers.json`
