...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the thesis four-group adaptive strategy experiment matrix."
    )
    parser.add_argument(
        "--config",
        type=Path,
        default=DEFAULT_ISSUE221_CONFIG_PATH,
        help="Thesis experiment matrix config JSON path.",
    )
    parser.add_argument(
        "--output-root",
        type=Path,
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Evaluate run-level outputs after execution. Ignored during dry-run.",
    )
    parser.add_argument(
        "--kg-path",
        type=Path,
...
def main() -> int:
    ...
    with _temporary_env("PLANNER_LLM_PROVIDER", args.planner_provider):
        ...
        print(f"[thesis-matrix] run_id={manifest['run_id']}")
        print(f"[thesis-matrix] freeze_id={manifest['freeze_id']}")
        print(f"[thesis-matrix] runs={len(manifest['runs'])}")
        print(f"[thesis-matrix] manifest={run_dir / 'runs_manifest.json'}")
        print(f"[thesis-matrix] log_index={run_dir / 'run_log_index.csv'}")
        if args.planner_provider:
            print(f"[thesis-matrix] planner_provider={args.planner_provider}")
        if not args.dry_run and args.evaluate:
            ...
            print(f"[thesis-matrix] evaluated_runs={len(result['run_level_results'])}")
            print(f"[thesis-matrix] summary_csv={run_dir / 'matrix_metrics_summary.csv'}")
            print(f"[thesis-matrix] validation_summary={run_dir / 'validation_summary.json'}")
    return 0
