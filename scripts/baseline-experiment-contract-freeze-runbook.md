# Baseline Experiment Contract Freeze Runbook
...
```bash
uv run python scripts/freeze_baseline_experiment_contract.py \
  --config configs/experiments/baseline_experiment_contract.json
```
...
- 任务集默认锚定到 `configs/experiments/vertical_ablation_experiment.json` 的四个任务键，并在本 issue 内冻结难度标签。
