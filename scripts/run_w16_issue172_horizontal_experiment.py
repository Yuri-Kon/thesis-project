...
from src.infra.issue172_horizontal_experiment import (
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run issue #172 external horizontal baseline experiment."
    )
    parser.add_argument(
        "--config",
        type=Path,
        default=DEFAULT_ISSUE172_CONFIG_PATH,
        help="Issue #172 horizontal experiment config JSON path.",
    )
    parser.add_argument(
        "--output-root",
        type=Path,
...
def main() -> int:
    ...
    with _temporary_env("PLANNER_LLM_PROVIDER", args.planner_provider):
        ...
        print(f"[issue172] run_id={manifest['run_id']}")
        print(f"[issue172] freeze_id={manifest['freeze_id']}")
        print(f"[issue172] runs={len(manifest['runs'])}")
        print(f"[issue172] manifest={run_dir / 'runs_manifest.json'}")
        print(f"[issue172] log_index={run_dir / 'run_log_index.csv'}")
        if args.planner_provider:
            print(f"[issue172] planner_provider={args.planner_provider}")
        if args.evaluate:
            ...
            print(f"[issue172] evaluated_runs={len(result['run_level_results'])}")
            print(f"[issue172] summary_csv={run_dir / 'horizontal_metrics_summary.csv'}")
            print(f"[issue172] validation_summary={run_dir / 'validation_summary.json'}")
    return 0
