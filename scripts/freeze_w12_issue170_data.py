...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Issue #170 dataset freeze: D-main / D-recovery / D-hitl + manifest.",
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional execution plan index from issue #169 for hard-dependency validation.",
    )
    parser.add_argument(
        "--allow-not-ready",
        action="store_true",
...
def main() -> int:
    ...
    print("Issue #170 data freeze completed")
    print(
        _json_dump(
            {
