...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run Issue #171 vertical experiment groups (A0-A6)."
    )
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("configs/experiments/w12_issue171_vertical_a0_a6.json"),
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional override of freeze manifest path (from issue #170).",
    )
    parser.add_argument(
        "--output-root",
        type=Path,
...
def main() -> int:
        ...
        for replicate in range(1, repeats + 1):
            for task in tasks:
                ...
                merged_metadata = deep_merge(
                    ...
                    {
                        "issue_id": issue_id,
                        "experiment": "w12_issue171_vertical",
                        "group_id": group_id,
                        "task_key": task_key,
                        "replicate": replicate,
                        "freeze_id": freeze_id,
    ...
    print(f"[issue171] run_id={run_id}")
    print(f"[issue171] freeze_id={freeze_id}")
    print(f"[issue171] runs={len(run_entries)}")
    print(f"[issue171] manifest={run_dir / 'runs_manifest.json'}")
    print(f"[issue171] log_index={run_dir / 'run_log_index.csv'}")
