# Vertical Ablation Experiment Runbook

## 1. 目标与范围

本实现对应 A0-A6 纵向对比与机制增量归因，并对齐：
...
- `configs/experiments/vertical_ablation_experiment.json`
...
```bash
uv run python scripts/run_vertical_experiment.py \
  --config configs/experiments/vertical_ablation_experiment.json \
...
```bash
uv run python scripts/evaluate_vertical_experiment.py \
