...
DEFAULT_CONFIG_PATH = Path("configs/experiments/execution_plan_freeze.json")
...
def _render_markdown(
    *,
    ...
    lines: list[str] = []
    lines.append("# Execution Plan Freeze Index")
    lines.append("")
...
def freeze_issue169_plan(
    *,
    ...
    # Acceptance checks for the frozen execution plan.
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Freeze experiment plan with dependency/timeline validation.",
    )
...
def main() -> int:
    ...
    print("Execution plan freeze completed")
    print(
        _json_dump(
            {
