...
from services.openfold3_rest_server.jobs import (
    DEFAULT_DEVICE,
    DEFAULT_MODEL_DIR,
    DEFAULT_PREDICT_BIN,
    JobError,
...
def _build_app(
    *,
    ...
    app = FastAPI(title="OpenFold3 REST Server", version="0.1.0")
    app.state.remote_base_dir = base_dir
    app.state.api_token = api_token
    app.state.model_dir = model_dir
    app.state.predict_bin = predict_bin
...
def create_app(
    *,
    ...
    predict_bin: Optional[str] = None,
    device: Optional[str] = None,
    run_job_func: Optional[Callable[..., None]] = None,
) -> FastAPI:
    base_dir = Path(
        remote_base_dir or os.getenv("OPENFOLD3_REST_BASE_DIR", "./output/remote/openfold3_jobs")
    )
    base_dir.mkdir(parents=True, exist_ok=True)
    token = api_token if api_token is not None else os.getenv("OPENFOLD3_REST_API_TOKEN", "")
    resolved_model_dir = model_dir or os.getenv("OPENFOLD3_MODEL_DIR", DEFAULT_MODEL_DIR)
    resolved_predict_bin = predict_bin or os.getenv("OPENFOLD3_PREDICT_BIN", DEFAULT_PREDICT_BIN)
    resolved_device = device or os.getenv("OPENFOLD3_DEVICE", DEFAULT_DEVICE)
