...
from services.openfold3_rest_server.openfold3_runner import (
    DEFAULT_DEVICE,
    DEFAULT_MODEL_DIR,
    DEFAULT_PREDICT_BIN,
...
def run_job(
    ...
    *,
    ...
    _write_json(
        status_path(base_dir, job_id),
        {"job_id": job_id, "status": "running", "updated_at": _now_iso()},
    )

    try:
        outputs_payload, metrics = run_openfold3_prediction(
            inputs=inputs,
            job_path=job_path,
            model_dir=model_dir,
        ...
        _write_json(
            status_path(base_dir, job_id),
            {"job_id": job_id, "status": "completed", "updated_at": _now_iso()},
        )
