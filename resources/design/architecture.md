...
### 算法闭环与 Planner 模型策略（补充）

基于 `docs/algorithm-and-llm/` 的结论，系统在架构层采用“候选集决策 + 状态机控制”而非单次文本规划。

- 规划最小单元是 `Top-K CandidateSet`，而非单个 Plan。
- 关键门控点由风险/成本阈值与策略配置共同决定是否进入 `WAITING_*`。
- 失败恢复必须遵循统一顺序：`retry -> patch -> replan`。
- 审计链路必须覆盖 `PendingAction -> Decision -> EventLog -> TaskSnapshot`。

Planner 模型采用双路调用策略：

- 默认：自研 Planner 专用模型（强调结构合法与可执行性）。
- 回退：外部强模型（仅替换模型调用层，不改变 FSM/HITL/Decision 契约）。
- 回退触发：连续 schema 不合法、候选可执行率下降、连续失败超阈值、风险长期偏高。
...
- PlannerAgent(LLM): 大脑，负责读任务、查数据库，生成候选计划并给出默认建议
...
1. 默认优先调用自研 Planner 专用模型生成候选集。
2. 候选经 schema/约束检查后进入排序与门控。
3. 若触发回退条件（schema 连续不合法、可执行率显著下降、连续失败超阈值、风险长期偏高），切换外部模型兜底。
4. 双路切换只影响模型提供方，不改变下游 `PendingAction / Decision / FSM` 路径。
...
4. PlannerAgent生成候选集合（Top-K）
   在这些信息的基础上，Planner会输出候选集（默认 K=3），每个候选包含:  
   - `structured_payload`（可执行 Plan/PlanPatch/Replan）
   - `score_breakdown`（可执行性、目标贴合、风险、成本等）
   - `risk_level` / `cost_estimate` / `explanation`
   同时会给出默认建议 `default_suggestion`
5. TaskAPI执行门控并固化方案
   - 若满足自动执行条件：固化默认候选，任务状态进入 `PLANNED`
   - 若触发人工门控：生成 `PendingAction(plan_confirm)`，任务状态进入 `WAITING_PLAN_CONFIRM`
   - 决策生效后再固化最终 Plan 并继续执行
