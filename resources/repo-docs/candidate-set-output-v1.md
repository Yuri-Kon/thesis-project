...
## Planner Top-K generation strategy

- Default `K=3` for `plan/patch/replan` candidate generation.
- Deterministic ranking key:
  - `score_breakdown.overall` (descending)
  - `priority` (`P0` preferred to `P1/P2/...`)
  - `capability_bucket`
  - `tool_id`
  - `candidate_id`
- Diversity policy: final Top-K is selected by capability-bucket round-robin from ranked candidates.
- Tool metadata minimum set is always attached on each candidate:
  - `tool_id`
  - `capability_id` (also used as capability bucket)
  - `io_type`
  - `adapter_mode`

### Degradation policy

- If unique deterministic candidates are fewer than requested `K`, return all available candidates.
- `default_recommendation` still points to the first ranked candidate.
- Candidate set `explanation` must include requested vs returned count and indicate degraded output.
...
## S1 Sequence Exploration Extension (#155)

S1-specific planner metadata contract is documented in:

- `docs/algorithm-and-llm/s1-sequence-exploration-contract.md`

This extension is additive and keeps CandidateSetOutput v1 compatibility.
