...
async function confirmIntake() {
  ...
  const acknowledged = [];
  const precheck = state.session.safety_precheck;
  if (precheck?.acknowledgement_required) {
    acknowledged.push(precheck.code);
  }
...
function renderSession() {
  ...
  renderSafetyPrecheck(session.safety_precheck);
...
function renderSafetyPrecheck(precheck) {
  ...
  ackRow.classList.toggle("hidden", !precheck?.acknowledgement_required);
  if (!precheck?.acknowledgement_required) {
    ack.checked = false;
  }
}
...
function renderConfirmAvailability(session) {
  ...
  const precheck = session.safety_precheck || { action: "ok" };
  const ack = byId("safety-ack-checkbox");
  const blocked =
    (session.missing_required_fields || []).length > 0 ||
    (session.ambiguous_fields || []).length > 0 ||
    (session.warnings || []).length > 0 ||
    precheck.action === "block" ||
    (precheck.acknowledgement_required && !ack.checked);
