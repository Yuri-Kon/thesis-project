...
DEFAULT_ISSUE172_CONFIG_PATH = Path(
    "configs/experiments/w16_issue172_horizontal_experiment.json"
)
DEFAULT_ISSUE172_OUTPUT_ROOT = Path("output/experiment/w12-expr-3/issue172-horizontal")
        ...
        for task in matrix["tasks"]:
            ...
            for replicate in range(1, repeat_count + 1):
                ...
                merged_metadata = deep_merge(
                    ...
                    {
                        "issue_id": issue_id,
                        "experiment": "w12_issue172_horizontal",
                        "group_id": group_id,
    ...
    try:
        ...
        if loop_budget <= 0 and context.status not in TERMINAL_INTERNAL_STATUSES:
            transition_task_status(
                ...
                reason="issue172_loop_budget_exhausted",
            )
...
def _auto_apply_horizontal_decision(
    *,
    ...
    decision = Decision(
        decision_id=f"{task.task_id}_{pending_action.pending_action_id}_issue172",
        task_id=task.task_id,
        pending_action_id=pending_action.pending_action_id,
        choice=choice,
        selected_candidate_id=candidate_id,
        decided_by="issue172_horizontal_runner",
        comment=comment,
    )
    if pending_action.action_type == PendingActionType.PLAN_CONFIRM:
...
def _build_issue172_markdown_report(
    *,
    ...
    if lines and lines[0] == legacy_header:
        lines[0] = f"# Issue #{issue_id} Horizontal Experiment Report (E0/E1/E2)"
