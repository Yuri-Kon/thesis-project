...
class FailureCode(str, Enum):
    ...
    # NVIDIA NIM 调用失败
    NIM_API_KEY_MISSING = "NIM_API_KEY_MISSING"  # NIM API key 缺失
    NIM_HTTP_4XX = "NIM_HTTP_4XX"  # NIM HTTP 4xx 错误
    NIM_HTTP_5XX = "NIM_HTTP_5XX"  # NIM HTTP 5xx 错误
    NIM_TIMEOUT = "NIM_TIMEOUT"  # NIM 请求超时
    NIM_NETWORK_ERROR = "NIM_NETWORK_ERROR"  # NIM 网络错误
    NIM_INVALID_RESPONSE = "NIM_INVALID_RESPONSE"  # NIM 响应格式无效
    NIM_UNEXPECTED_ERROR = "NIM_UNEXPECTED_ERROR"  # NIM 未预期异常
    NIM_AUTH_FAILED = "NIM_AUTH_FAILED"  # NIM 认证失败（API key 无效/过期）
    NIM_QUOTA_EXCEEDED = "NIM_QUOTA_EXCEEDED"  # NIM 配额/速率限制
    NIM_MODEL_NOT_FOUND = "NIM_MODEL_NOT_FOUND"  # NIM 模型不可用
    NIM_INVALID_INPUT = "NIM_INVALID_INPUT"  # NIM 输入验证失败
    NIM_MODEL_ERROR = "NIM_MODEL_ERROR"  # NIM 模型内部错误

    # 通用失败码
    CANDIDATE_SCHEMA_INVALID = "CANDIDATE_SCHEMA_INVALID"  # 候选 schema / 元数据不合法
    CANDIDATE_TOOL_UNAVAILABLE = "CANDIDATE_TOOL_UNAVAILABLE"  # 候选引用工具不存在或适配器不可用
    CANDIDATE_IO_CLOSURE_BROKEN = "CANDIDATE_IO_CLOSURE_BROKEN"  # 候选 I/O 闭包断裂
    CANDIDATE_PARAMS_INVALID = "CANDIDATE_PARAMS_INVALID"  # 候选参数类型/范围/必填项不合法
    CANDIDATE_RESOURCE_CONSTRAINT = "CANDIDATE_RESOURCE_CONSTRAINT"  # 候选超出基础资源约束
    CANDIDATE_ADAPTER_UNSUPPORTED = "CANDIDATE_ADAPTER_UNSUPPORTED"  # 候选工具后端接入方式不受支持
