...
六阶段是能力分层，而非固定流水线。质量门禁失败可回到序列探索，目标评分不足可回到结构条件精修。CEBRA-WP 的作用是在可替代路径间进行约束化选择：证据不足时延后高代价步骤，局部失败可修复时优先 `patch_local`，结构性失败时转向 `suffix_replan` 或 stop。
...
CEBRA-WP 的动机来自三类研究：部分可观测规划强调 belief-state[16,23]，预算约束研究要求把成本和风险作为一等变量[24]，Tree of Thoughts 与 Reflexion 提供多候选搜索和失败反馈思路[20,21]。本文将这些思想约束到可执行工具链规划中，使候选、恢复动作和人工确认均以结构化对象表达。
...
其中，${Conf}(\pi)$选置信度， $RiskScore(\pi)$ 和 $CostScore(\pi)$ 为候选的风险与成本质量分数，$RecoveryScore(\pi)$ 表示候选恢复友好度，$ActionBias\left( \pi,x_{t} \right)$ 表示候选与当前恢复动作偏好的匹配程度。其直观含义是：当 $p_{\text{sf}}$ 和预算压力升高时，算法降低高成本、低可恢复性候选的排序；当 $p_{\text{succ}}$、$r_{\text{rec}}$ 和 $e_{\text{suf}}$ 较高时，算法提高继续执行或低风险候选的排序。
