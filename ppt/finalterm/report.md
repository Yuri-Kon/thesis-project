...
本文档对应当前 `ppt/finalterm/2022112879-郑彦文-毕业设计答辩.pptx`。当前答辩演示文稿共 20 页，其中第 1-15 页为正式汇报，第 16-19 页为备份页，第 20 页为结束页。正式汇报建议控制在 8-10 分钟，目标时长约 9 分钟 30 秒，问答约 5 分钟。
...
|---:|---|---:|---|
| 1 | 封面 | 10 秒 | 题目与工作定位 |
| 2 | 目录 | 15 秒 | 给出汇报顺序，提醒算法与验证是重点 |
| 3 | 汇报结构与核心问题 | 20 秒 | 本文围绕一个运行时控制问题展开 |
...
这页给出本次汇报的顺序。前两部分先说明为什么需要工作流规划机制，以及系统架构如何约束算法动作；中间重点讲 CEBRA-WP，也就是约束与证据感知、信念引导、恢复自适应的工作流规划，包括候选过滤、静态评分、轻量信念状态和恢复动作选择；后面说明这些算法对象如何落到实现，并用系统测试和 84 次运行消融实验验证机制是否成立。
...
本次答辩围绕一个问题展开：高代价、可失败的蛋白质设计工具链，如何在执行过程中做可解释的规划与恢复决策。前两部分先说明工程约束和系统支撑，中间重点讲核心算法 CEBRA-WP，也就是约束与证据感知、信念引导、恢复自适应的工作流规划，后面再说明它如何落到代码，以及用哪些测试和消融实验验证。

这里的核心判断是，本文的贡献集中在运行时工作流规划：先约束候选可执行，再依据运行时证据选择继续执行、局部修补、后缀重规划或受保护停止。
...
这页展示的是系统的整体架构。它不是单纯把模块堆在一起，而是说明大模型规划如何被收束到一个可控制、可恢复的工程流程里。
...
运行时阶段的难点是观测不完整。一次工具失败不一定说明整条路线都错了，一次成功也不能说明后续一定可靠。因此系统没有把运行时事件直接变成动作，而是先更新一个轻量信念状态，也就是五维运行时状态摘要。

这个状态来自五类信息：步骤执行结果中的输出、指标和错误；安全检查结果中的风险标记；失败上下文和修补/重规划历史；预算与任务进度；以及人在环决策记录。它被压缩成五个变量：`p_success` 表示继续成功概率，`p_structural_failure` 表示结构性失败压力，`recovery_margin` 表示恢复余量，`expected_remaining_cost` 表示预期剩余成本，`evidence_sufficiency` 表示证据充分度，也就是证据是否足以进入高代价步骤。
...
屏幕重点：上方候选运行时效用 `U_pi` 和动作效用，下方四类动作。

建议口播：

动态阶段有两个输出。第一个是候选重排：静态分或后验目标分作为基础，运行时状态只做有界修正，也就是 `U_pi = clip(S_post + Delta, 0, 1)`。这里的 `Delta` 可以口播为“运行时修正项”，`clip` 表示把分数限制在 0 到 1 之间；它们不会改变硬过滤结果，只影响已通过硬过滤候选的排序。
...
这里一共设计了 13 个系统测试用例，12 个通过，1 个命令行相关用例部分通过，没有出现静默崩溃。中间五个框对应覆盖范围：接口和任务录入，有限状态机、人在环决策与快照，前端和命令行，端到端工具链，以及异常、安全和恢复路径。
...
从完成情况看，静态组 21 次全部完成，另外三组各有 1 次失败，所以本文不声称 CEBRA-WP 显著提升成功率。更关键的是机制差异。第一，只有轻量信念状态组 21/21 产生 RuntimeState，说明五维运行时状态可以被持续记录和观察。第二，从高代价调用看，固定阈值门控组是 28 次，动态恢复组和轻量信念状态组都是 20 次，相比固定阈值门控减少 28.6%。这说明动态机制在减少无效高代价调用方面有证据。
...
- `S_static` 对应 `score_breakdown.overall`，表示静态候选效用，口播名是“静态评分”。
- `x_t` 对应 `RuntimeState`，表示五维运行时状态，口播名是“轻量信念状态”。
- `Delta` 对应 `runtime_adjustment`，表示有界运行时修正。
- `U_pi` 对应候选运行时效用，表示重排后的候选分数。
