...
本文档对应当前 `ppt/finalterm/2022112879-郑彦文-毕业设计答辩.pptx`。当前答辩演示文稿共 18 页，其中第 1-13 页为正式汇报，第 14-17 页为备份页，第 18 页为结束页。正式汇报建议控制在 8-10 分钟，目标时长约 9 分钟 20 秒，问答约 5 分钟。
...
### 第 12 页：验证：系统测试 + 84 次运行消融

回答的问题：如何验证系统有效，但不过度夸大。

屏幕重点：上方五个数字，右侧 84 次运行的机制证据。

建议口播：

验证分两部分。第一部分是系统测试，共 13 个测试用例，12 个通过，1 个命令行用例部分通过，覆盖接口、任务录入、人在环决策/有限状态机/快照、前端和命令行、端到端工具链，以及异常、安全和恢复路径。第二部分是 84 次运行的消融实验，总体 81/84 进入完成终态，轻量信念状态组 21/21 产生运行时状态，说明运行时状态可以被记录和观察。

从高代价调用看，固定阈值门控组高代价调用是 28 次，动态恢复组和轻量信念状态组都是 20 次，相比固定阈值门控组减少 28.6%。这说明动态机制在减少无效高代价调用方面有证据。
