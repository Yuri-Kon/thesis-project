...
这页给出本次汇报的顺序。前两部分先说明为什么需要工作流规划机制，以及系统架构如何约束算法动作；中间重点讲 CEBRA-WP，也就是约束与证据感知、信念引导、恢复自适应的工作流规划，包括候选过滤、静态评分、轻量信念状态和恢复动作选择；后面说明这些算法对象如何落到实现，并用系统测试和 84 次运行消融实验验证机制是否成立。最后补充评阅意见与论文修改说明。
